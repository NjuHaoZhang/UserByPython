...
# TODO 需要重构重定向的方法 ，使用lesson_5的方法，返回一个403重定向的返回码来实现
...
# TODO：session的用法也不好。重构方法：session中只存放username,然后点击profile的链接（或者直接地址栏中输入profile）
# 在route_profile中直接查session得到username，然后再通过username查id和note.
def route_profile(request):
    ...
    if userinfo_dict != '':
        ...
        log('第一次login 的响应', r)
        return r.encode(encoding='utf-8')
    else:  # 没有登录就重定向  # TODO：我说的就是这里的重定向其实应该用 Location:/login及返回403状态码来实现
        # log("还没有登录，需要重定向")
